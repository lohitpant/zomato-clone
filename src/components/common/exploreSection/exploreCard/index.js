    ...
    return (
        <div className='explore-card cur-po'>
            ...
            <div className='res-row'>
                ...
                {rating && <div className='res-rating absolute-center'>{rating}<AiFillStar className='absolute-center star' /> </div>}
            </div>
            <div className='res-row'>
                {cuisines.length && <div className='res-cuisine'>
                    {cuisines.map((item, i) => {
                        return (
                            <span key={i} className='res-cuisine-tag'>{item}{i !== cuisines.length - 1 && ", "}</span>
                        )
                    })}
                </div>}
